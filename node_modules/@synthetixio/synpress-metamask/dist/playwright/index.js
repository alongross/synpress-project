...
import { createTempContextDir, removeTempContextDir, CACHE_DIR_NAME, ensureCacheDirExists, downloadFile, unzipArchive } from '@synthetixio/synpress-cache';
...
var fixCriticalError = async (page) => {
...
async function retryIfMetaMaskCrashAfterUnlock(page) {
  ...
  if (!isHomePageLogoVisible && !isPopoverVisible) {
    if (await page.locator(CrashPage.selectors.header).isVisible()) {
      const errors2 = await page.locator(CrashPage.selectors.errors).allTextContents();
      console.warn(["[RetryIfMetaMaskCrashAfterUnlock] MetaMask crashed due to:", ...errors2].join("\n"));
      ...
      try {
        ...
        if (e instanceof errors.TimeoutError) {
          throw new Error(
            ["[RetryIfMetaMaskCrashAfterUnlock] Reload did not help. Throwing with the crash cause:", ...errors2].join(
              "\n"
            )
          );
        }
